...
Essa seção foi atualizada com o **log real de `dbt test`**, rodado no ambiente local (Windows/PowerShell, 09/09/2026, 44 testes no total). Antes disso, os números tinham sido só validados manualmente em DuckDB (mesma limitação de rede da seção 4.2) — a rodada real confirmou a maioria, corrigiu um bug real que só apareceria rodando de verdade, e deixou uma pequena divergência ainda sob investigação (ver 6.3).

### 6.1 Bug real encontrado só na rodada de `dbt test`

Os 5 testes `unique` de `staging/schema.yml` (`stg_partners`, `stg_sessions`, `stg_searches`, `stg_bookings`, `stg_cancellations`) rodaram como **error** e falharam (118, 79, 2, 236, 342 duplicatas — exatamente as contagens da tabela da seção 2), mesmo esse documento dizendo desde a primeira versão que staging deveria ser `severity: warn`. O `schema.yml` tinha o teste declarado (`tests: [not_null, unique]`) mas nunca tinha o `config: severity: warn` de fato aplicado — documentação e código divergindo de novo, mesmo padrão do achado 4.1, só que dessa vez só a rodada real do `dbt test` pegou (a revalidação manual em DuckDB não simula severidade de teste, só a query em si). **Corrigido**: `unique` virou uma entrada própria com `config: severity: warn` em cada uma das 5 colunas. Também aproveitamos pra corrigir um `[WARNING][MissingArgumentsPropertyInGenericTestDeprecation]` que apareceu no log (argumentos de teste genérico precisam ficar aninhados em `arguments:`, não soltos) — não travava nada, mas ia virar erro em versão futura do dbt.

### 6.2 Testes genéricos/`dbt_expectations` novos (`schema.yml`, severity `warn`) — confirmados via `dbt test` real
...
Todos batem exatamente com o que tinha sido validado manualmente.

### 6.3 Testes singulares novos (`tests/*.sql`, severity `warn`) — confirmados via `dbt test` real

| Arquivo | O que verifica | Validado manualmente (DuckDB) | `dbt test` real |
|---|---|---|---|
| `teste_reservas_duplicadas_usuario_dia.sql` | Mesmo usuário com >1 reserva no mesmo dia | 10 | **10** ✅ |
| `teste_duracao_sessao.sql` | Sessão com duração > 6h | 571 | **571** ✅ |
...
4 dos 5 bateram exatamente. O `teste_clique_sem_resultado` divergiu em 32 casos (1.163 no ambiente real vs. 1.195 na validação manual) — a lógica do teste está correta (mesma query rodando nos dois lugares), a suspeita é diferença de carregamento do CSV bruto entre o `external_location` do dbt-duckdb e o `read_csv_auto` usado na validação manual. Não muda a existência do achado (ainda é o teste mais volumoso depois do `refund_amount`), só o número exato — está sendo investigado, e o `dbt test` real (1.163) é o número que vale como oficial até resolvermos.
...
| `fct_bookings` | 1 linha por `booking_id` | Reservas, incremental, já validadas (parceiro ativo, `total_amount > 0` p/ confirmed/completed, deduplicado) |
| `fct_sessions` | 1 linha por `session_id` | Sessões de navegação, incremental, sem bot, deduplicado |

### 7.2 Glossário de métricas

Definições oficiais do `data_dictionary.md`, com a fórmula exata usada nos modelos/queries deste projeto (não só a definição textual do case):

| Métrica | Definição | Como é calculada aqui |
|---|---|---|
| **Taxa de Conversão** | Proporção de sessões que resultam em ao menos uma reserva confirmada | `fct_bookings` (status confirmed/completed, fora de `int_cancellations`) / `int_sessions`, por país×device — ver Q1 (seção 5) |
...
| **Taxa de Cancelamento** | % de reservas canceladas sobre o total de reservas confirmadas | `int_cancellations` (via `booking_id`) / `fct_bookings` com `status in ('confirmed','completed')`, por parceiro — ver Q5 |
...
Não existe um SLA formal definido no `data_dictionary.md` do case (ele não dá números de threshold), então os limiares abaixo foram definidos por nós, a partir do que o profiling mostrou como "normal" nesse dataset — documentando o raciocínio, não só o número.

### 8.1 Frescor (freshness)

- `fct_bookings` e `fct_sessions` são incrementais (`delete+insert`, filtrando por `booked_at`/`started_at` mais recente que o já carregado). Num cenário produtivo, o SLA proposto é: **dado é considerado "atual" se o `dbt run` incremental rodou nas últimas 24h**. Rodadas incrementais paradas por mais de 24h devem disparar alerta — não temos orquestração (Airflow/Cron) configurada neste case, mas é o gancho natural pra isso.
...
- Campos com nulo declarado como esperado no dicionário (`tier` 14%, `utm_source` 29%, `payment_method` 25%, etc.) — **tolerância = a % declarada no `data_dictionary.md` ± 3 pontos percentuais**. Fora dessa faixa, é sinal de mudança de comportamento na origem do dado (ex: um novo sistema deixou de preencher um campo que antes preenchia).
...
| Sessão > 6h de duração | 571 (em `int_sessions`) | Alerta se ultrapassar 1% do total de sessões — hoje está em ~0,5% |
...
Juntando tudo: **um mart é considerado confiável pra uso em decisão de negócio quando (1) a última rodada incremental tem menos de 24h, (2) todos os testes `severity: error` passaram, e (3) nenhum teste `severity: warn` ultrapassou o threshold da tabela 8.4.** Isso é o critério que a gente propõe pra virar um gate de CI (ex: Buildkite/GitHub Actions rodando `dbt build` a cada PR e bloqueando merge se `error` falhar), não implementado neste case por restrição de tempo, mas é a recomendação natural de próximo passo.
...
|---|---|---|---|
| `user_id` | `raw_sessions`, `raw_bookings`, `dim_users`, `fct_bookings`, `fct_sessions` | **Identificador indireto** | É um UUID, não carrega nome/e-mail/documento diretamente — mas permite religar todo o histórico de navegação e reserva de uma pessoa se cruzado com outra fonte (ex: sistema de autenticação) |
| `contact_email` | `raw_partners`, `dim_partners` | **Dado de contato (Restrito)** | E-mail operacional de empresa parceira (ex: `ops@localiza.com`), não de pessoa física — mas ainda é um dado de contato que não deveria vazar em relatório externo sem necessidade |
...
### 9.2 Tratamento proposto

- **`user_id`**: já é pseudonimizado na origem (UUID, não o identificador real do usuário). Tratamento adicional proposto: **controle de acesso por necessidade** — só modelos/dashboards que precisam de análise por usuário individual (cohort, LTV) devem ter acesso à coluna crua; dashboards agregados (ex: funil por país×device) não precisam expor `user_id` nenhuma vez. Se algum dia esse dado for exportado pra fora do ambiente controlado (ex: CSV pra um parceiro externo), propomos hash adicional (`sha256(user_id || salt)`) antes da saída, pra não expor nem o UUID interno.
- **`contact_email`** (`dim_partners`): classificado como **Restrito** — mascaramento proposto para qualquer camada de consumo fora do time de operações/parcerias: mostrar só o domínio (`ops@***.com`) em relatórios de BI de uso mais amplo, e-mail completo só pro time que já opera diretamente com o parceiro.
- **Nenhuma anonimização/exclusão foi necessária** pros marts construídos no D1 — nenhum dos 4 (`dim_partners`, `dim_users`, `fct_bookings`, `fct_sessions`) expõe PII de pessoa física. A única coluna com `contains_pii: true` nos marts é `user_id`, e ela é tratada como identificador pseudonimizado, não como PII direta.
- **Retenção**: não há política de retenção definida no case nem dado suficiente pra propor um número específico com segurança — fica registrado como gap: numa implementação real, valeria definir por quanto tempo `user_id`/sessão ficam identificáveis antes de serem agregados/anonimizados (prática comum: 12-24 meses pra dado comportamental de e-commerce/travel).

## 10. Limitações e itens em aberto

- Todos os achados de qualidade encontrados até aqui (múltiplas reservas mesmo usuário/dia, `days_before_pickup` negativo, outliers de `total_amount`, categoria `compact` fora do domínio, sessões >6h, clique sem resultado, `refund_amount > total_amount`) agora têm teste formal em `severity: warn`, confirmado via `dbt test` real (seção 6) — mas continuam **monitorados, não bloqueados/filtrados** no fato (exceto `refund_amount` e `total_amount<=0`, que já eram filtrados desde o D1). É uma decisão de escopo consciente: são sinais de possível problema, não certezas, e excluir automaticamente arriscaria jogar fora dado legítimo.
- ~~Pendência: confirmar `ERROR=0` depois do fix de severidade~~ — **resolvido**: `dbt test` real em 09/09/2026 deu `PASS=30 WARN=14 ERROR=0` (seção 6.1).
- **Pendência real:** o `teste_clique_sem_resultado` divergiu 32 casos entre a validação manual (1.195) e o `dbt test` real (1.163) — ver seção 6.3. Suspeita é diferença de carregamento de CSV entre ambientes (`external_location` do dbt-duckdb vs. `read_csv_auto` usado na validação manual), ainda não confirmada com uma query de diagnóstico direto no ambiente real. Não bloqueia a entrega — o achado em si está confirmado e testado, só o número exato de violações tem essa pequena divergência entre ambientes, registrada com transparência em vez de escondida.
- Política de retenção de dado pessoal (`user_id`) não tem um número definido — fica como gap explícito na seção 9.2, não uma lacuna escondida.
...
- SLA de dados (seção 8) não está implementado como gate de CI real — é a definição/proposta de threshold, não a automação em si. Automatizar isso (ex: pipeline falhar/alertar quando os thresholds da seção 8.4 forem ultrapassados) é next step natural, fora do escopo de tempo deste case.
